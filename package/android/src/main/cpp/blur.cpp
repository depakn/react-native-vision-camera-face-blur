...
void stackBlur(int* pix, int w, int h, int radius) {
  ...
  for (int y = 0; y < h; y++) {
    ...
    for (int x = 0; x < w; x++) {
      pix[y * w + x] =
          ((sum[0] / div) << ALPHA_CHANNEL) | ((sum[1] / div) << RED_CHANNEL) | ((sum[2] / div) << GREEN_CHANNEL) | (sum[3] / div);
...
extern "C" JNIEXPORT void JNICALL Java_com_mrousavy_camera_core_FaceDetectionRecorder_nativeStackBlur(JNIEnv* env, jobject /* this */,
                                                                                                      jintArray pixelsArray, jint width,
